...
def compute_meridional_gradients(z500_data, delta_lat=15.0, central_lat_min=None, central_lat_max=None):
    """
    ...
    central_lat_min : float, optional
        Minimum central latitude for gradient computation. If None, automatically
        determined as data_lat_min + 30° (default: None).
    central_lat_max : float, optional
        Maximum central latitude for gradient computation. If None, automatically
        determined as data_lat_max - 15° (default: None).
    ...
    # Determine valid central latitudes
    # Need φ-30° for GHGS2, φ-15° for GHGS, φ+15° for GHGN
    if central_lat_min is None:
        central_lat_min = data_lat_min + 2 * delta_lat  # Need φ-30° to exist
    if central_lat_max is None:
        central_lat_max = data_lat_max - delta_lat      # Need φ+15° to exist
    
    # Verify data has sufficient range
    lat_min_required = central_lat_min - 2 * delta_lat
    lat_max_required = central_lat_max + delta_lat
    
    # Check if data range is sufficient (with small tolerance for grid resolution)
    # Allow up to 5° tolerance to account for grid resolution differences
    tolerance = 5.0
    
    if data_lat_min > lat_min_required + tolerance:
        raise ValueError(
            f"Input data latitude range [{data_lat_min:.1f}, {data_lat_max:.1f}] "
            f"is insufficient for central range [{central_lat_min:.1f}, {central_lat_max:.1f}]. "
            f"Need data starting at approximately {lat_min_required:.1f}°N or lower "
            f"(data starts at {data_lat_min:.1f}°N)."
        )
    
    if data_lat_max < lat_max_required - tolerance:
        raise ValueError(
            f"Input data latitude range [{data_lat_min:.1f}, {data_lat_max:.1f}] "
            f"is insufficient for central range [{central_lat_min:.1f}, {central_lat_max:.1f}]. "
            f"Need data extending to approximately {lat_max_required:.1f}°N or higher "
            f"(data extends to {data_lat_max:.1f}°N)."
        )
    
    # Adjust central_lat_max based on available data
    # We need φ+15° to exist, so max central lat = data_lat_max - delta_lat
    effective_central_lat_max = min(central_lat_max, data_lat_max - delta_lat)
    
    # Warn if we had to reduce the range
    if effective_central_lat_max < central_lat_max:
        print(f"   ⚠️  Warning: Requested central_lat_max={central_lat_max:.1f}°N, "
              f"but data only extends to {data_lat_max:.1f}°N. ")
        print(f"   Adjusted to effective_central_lat_max={effective_central_lat_max:.1f}°N "
              f"(need φ+15° = {effective_central_lat_max + delta_lat:.1f}°N)")
    elif data_lat_max < lat_max_required - 2.0:
        print(f"   ⚠️  Warning: Data extends to {data_lat_max:.1f}°N, "
              f"which is close to the required {lat_max_required:.1f}°N. "
              f"Some high-latitude gradients may be limited.")
    
    print(f"\n🌍 GLOBAL ABS: Computing gradients for specified latitudes")
    print(f"   Data range: [{data_lat_min:.1f}°N, {data_lat_max:.1f}°N]")
    print(f"   Central latitudes: [{central_lat_min:.1f}°N, {effective_central_lat_max:.1f}°N]")
    print(f"   (Requires φ-30° and φ+15° to exist)")
    
    # Select all valid central latitudes for output
    central_lat_mask = (z500_data.lat >= central_lat_min) & (z500_data.lat <= effective_central_lat_max)
...
def abs_blocking_frequency(z500_data,
                           ...
                           central_lat_max=75.0,
                           delta_lat=15.0):
    """
    Perform complete ABS (Atmospheric Blocking System) blocking detection workflow.
    FIXED LATITUDE VERSION: Detects blocking at specified central latitudes independently
    of the input data range.
    ...
    2. Compute meridional gradients (GHGN, GHGS, GHGS2) for FIXED central latitudes
       (default: 45-75°N). Input data must extend to central_lat_min-30° and central_lat_max+15°.
    3. Apply longitudinal averaging (Δ/2 = 7.5°)
    4. Apply ABS criteria at fixed latitudes (A: gradient reversal, B: subtropical filter, 
                                              C: spatial extent, D: temporal persistence)
    5. Calculate blocking frequency at the specified central latitudes
    ...
    lat_min : float, optional
        ...
        Must be ≤ central_lat_min - 30° to provide data for gradient computation
    lat_max : float, optional
        Maximum latitude for data preparation in degrees N (default: 90.0)
        Must be ≥ central_lat_max + 15° to provide data for gradient computation
    central_lat_min : float, optional
        Minimum central latitude for blocking detection in degrees N (default: 45.0)
        Blocking is detected from this latitude northward
    central_lat_max : float, optional
        Maximum central latitude for blocking detection in degrees N (default: 75.0)
        Blocking is detected up to this latitude
    delta_lat : float, optional
        Latitude spacing for gradient computation in degrees (default: 15.0)
    ...
    blocking_frequency : xarray.DataArray
        ...
        Covers the FIXED central latitude range [central_lat_min, central_lat_max].
        Default range: 45°N to 75°N.
    
    Notes
    -----
    - Central latitudes are FIXED at specified values (default: 45-75°N)
    - Extending lat_min/lat_max provides data for gradients but doesn't change detection range
    - This prevents distortion of blocking patterns when adding extra data coverage
    - Input data must extend to [central_lat_min - 30°, central_lat_max + 15°]
    - n_lon is automatically inferred from the input data
    - The function prints progress information during execution
    
    Examples
    --------
    >>> # Fixed-latitude blocking detection in DJF season (45-75°N)
    ...
    >>> 
    >>> # Extend data coverage without changing detection range
    >>> # This detects blocking at 45-75°N but uses data from 0-90°N for gradients
    >>> blocking_freq = abs_blocking_frequency(
    ...     z500_djf, 
    ...     lat_min=0.0,         # Extended data for gradient computation
    ...     central_lat_min=45.0, # Blocking still detected at 45-75°N
    ...     central_lat_max=75.0
    ... )
    >>> 
    ...
    # Step 2: Compute meridional gradients at fixed central latitudes
